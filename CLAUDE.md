...
- **analysis/H18_HE_follicles.ipynb** — H&E cohort (27 donors, 9/9/9 balanced, separate from fluorescent set): per-donor follicle morphometry by rs3184504 (count, area, density, circularity, NN distance). Original 5-metric scan was null (closest p=0.064 for area-fraction C/T vs T/T). Notebook also contains a power-aware reporting block (bootstrap 95% CIs + MDE r≈0.75 at n=9/9) and centroid-based spatial features (Clark-Evans R, area Gini, centrality) — Clark-Evans R shows a borderline T-allele-dosage trend (ρ=−0.34, p=0.079) toward more clustered follicles. Uses `Spleen_rs3184504_Genotypes.xlsx` and `27_donor_spleen_measurements1.csv`. Appendix sections live in `scripts/append_H18_power_analysis.py` and `scripts/append_H18_spatial_features.py` (idempotent re-runs).
